...
const {ModuleMap: {MODULE, METHOD_NAME, MODULE_NAME}}  = require ('require-sliced')
...
class Job extends EventEmitter {
	...
	async outcome () {
		...
		try {
			...
			const {app: {methodSelector, modules}, rq} = this

			const moduleName = methodSelector.getModuleName (rq); if (moduleName === null) return

			const method = modules.getMethod (moduleName, methodSelector.getMethodName (rq))

			this.module = method [MODULE]

			await this.broadcast ('method', method [METHOD_NAME])
