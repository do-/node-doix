...
class Job extends EventEmitter {
	...
	get isPending () {
		...
		return this.src.pending.has (this)

	}

	set isPending (value) {

		const {pending} = this.src

		if (value) pending.add (this); else pending.delete (this)
	...
	async outcome () {
		...
		try {
			...
			const minLatency = this [MIN_LATENCY]; if (minLatency === Infinity) return

			const lag = this.getTimeLeft (minLatency), finish = () => this.emit ('next')
	...
	spawn (request, options = {}) {
		...
		return this.app.spawn (request, {...options, parent: this})
	...
	* resources (clazz) {
	...
	async terminate () {
		...
		return new Promise (ok => this.once ('finish', () => {
	...
	async ensureLastJob (maxIterations = 100) {

		for (let i = 0; i < maxIterations; i ++) { // do our best to end all pending jobs gracefully

			const todo = []; for (const src of this.app.jobSources.values ())

				for (const job of src.pending)

					if (job !== this) todo.push (job.terminate ())

			if (todo.length === 0) break

			todo.push (new Promise (ok => setTimeout (ok, 10)))
