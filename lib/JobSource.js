...
class JobSource extends EventEmitter {
	...
	constructor (app, o = {}) {
		...
		this.app = app
		this.jobClass = o.jobClass ?? app.jobClass
		...
		if (app.jobSources.size === 0) app.defaultJobSource = this
	...
	createJob (request = {}, options) {
		...
		const job = new (this.jobClass) ({...this.request, ...request}, options)
...

		{
			...
			job.on ('next', () => job.isPending = false)
	...
	async exec (request, options) {
	 	...
	 	return this.createJob (request, options).outcome ()

	}

	spawn (request, options) {

	 	return this.createJob (request, options).run ()
	...
	async ensureNoPendingJobs ({maxIterations, caller}) {

		for (let i = 0; i < (maxIterations ?? 100); i ++) { // do our best to end all pending jobs gracefully

			const todo = []; for (const job of this.pending)
				
				if (job !== caller)
					
					todo.push (job.terminate ())
