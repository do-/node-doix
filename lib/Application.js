...
class Application extends EventEmitter {
	...
	constructor (o) {
		...
		for (const [k, v] of Object.entries (o)) if (v !== undefined) switch (k) {
			...
			case 'jobClass':
				if (v == null) throw new TypeError ('When set, jobClass must not be empty')
				const t = typeof v; if (t !== 'function') throw new TypeError (`When set, jobClass must be a class, not a ${t}`)
				const {prototype} = v; if (v !== Job && !(prototype instanceof Job)) throw new TypeError (`When set, jobClass must be a Job descendant`)
				this.jobClass = v
				break
		...
		if (!('jobClass' in this)) this.jobClass = Job
	...
	getMethod (request) {
		...
		const moduleName = namingConventions.getName ('module', request); if (moduleName == null) return
	
		return modules.getMethod (moduleName, namingConventions.getName ('method', request))

	}

	async exec (request, options) {

		return this.defaultJobSource.exec (request, options)

	}

	spawn (request, options) {

		return this.defaultJobSource.spawn (request, options)

	}

	* jobSourceClosings () {

		for (const src of this.jobSources.values ()) yield src.close ()
